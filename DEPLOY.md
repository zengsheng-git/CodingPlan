...
## ☁️ 部署到 Cloudflare Pages (Kimi 代理, 当前为演示数据)

> ⚠️ **现状告警**:Cloudflare 方案**未能让 Kimi 真实查询可用**。
> Kimi 上游自身在 Cloudflare 后面,拦截了来自 CF Workers 数据中心 IP 的请求("CF 拦 CF"死结)。
> 目前 CF Pages 上的 Kimi 返回的是**演示假数据**(带"演示数据"横幅),仅用于 UI 预览。
> 完整背景、原理、尝试过程、未来恢复方案见 **[KIMI-PROXY.md](KIMI-PROXY.md)**。
...
**原计划**: 同一个仓库再部署一份到 Cloudflare Pages, 用 [Pages Functions](https://developers.cloudflare.com/pages/functions/) 跑一个轻量代理函数, 接管 `/api/kimi-usages` 路径转发到 Kimi 上游。**实际结果**:代理代码正确,但被 Kimi 自身的 Cloudflare 拦截,改为返回演示假数据。
...
│  🟡 Kimi (返回演示假数据, 真实查询被 Kimi 的 CF 拦截)       │
└─────────────────────────────────────────────────────────────┘
```
